...
function getBlockNumber(){
    var params = [];
    // params.push(0);
    RPC_POST("jt_blockNumber", params, function(error, data){
        var result = {};
...
function send_request_get(url, callback){
    utility.sendRequest(url, "GET", PARSER_TEXT_JSON, null, null, null, callback);
}

function send_request_post(url, data, callback){
    utility.sendRequest(url, "POST", PARSER_TEXT_JSON, data, null, null, callback);
}

function sendRequest(url, method, parser, rawData, username, password, callback){
    //default is JSON
...
function RPC_POST(method, params, callback){
    ...
    sendRequest(url, request_method, parser_text, data, null, null, function(error, res){
        var result = {};
        if(res){
