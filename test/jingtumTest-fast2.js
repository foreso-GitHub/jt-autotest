...
async function run(){
  for(let mode of modes){
    ...
    // await testFastSend(server)
    // Promise.resolve(simplePromise()).then(function (value){
    //   logger.debug(value + "111")
    //   // return null
    // })

    let a = await simplePromise().then(function (value){
      logger.debug(value + "111")
      return "aaa"
      // return null
    })
...
async function executeDescribe(server, description){
  let finishCount = 0
  let cases = description.testCases
  // describe(description.title, async function () {
    for (let i = 0; i < cases.length; i++) {
      let testCase = cases[i]
      if(testCase.type === 'it'){
        await executeCase(server, testCase)
      }
      else if (testCase.type === 'describe'){
        await executeDescribe(server, testCase)
      }
      else{
...
async function executeCase(server, testCase){

  logger.debug("=== executeCase 1.1")
  return new Promise(async function (resolve) {
    logger.debug("=== executeCase 1.2")
    await Promise.resolve(sendTxWithSequence2(server, testCase.params)).then(function (response) {
      logger.debug("=== executeCase 1.3")
      testCase.response = response
      logger.debug("=== " + JSON.stringify(response))
      resolve(testCase)
    })
    logger.debug("=== executeCase 2.2")
  })
  logger.debug("=== executeCase 2.1")
...
async function testFastSend(server){
  let description = createCases()

  // await executeCase(server, description.testCases[0])
  // executeDescribe(server, description)

  let from = addresses.sender1.address
  ...
  //   logger.debug("!!!" + error)
  // })
  logger.debug("testFastSend")
  return null
}

async function sendTxWithSequence2(server, txParams){

  getSequence(server, txParams.from).then(function (sequence){
    Promise.resolve(server.responseSendTxWithSequence(
        txParams.from,
        txParams.secret,
        sequence,
        txParams.to,
    ...
    // resolve(response)
    logger.debug("sendTxWithSequence2 done!!!")
  }).catch(function (error) {
    logger.debug("!!!" + error)
  })
...
function checkResponse(response, needPass){
  expect(response).to.be.jsonSchema(schema.RESPONSE_SCHEMA)
  expect(response.status).to.equal(needPass ? status.success: status.error)
}

function checkSendTxResponse(response, needPass, isErrorInResult, expectedError){
  checkResponse(response, needPass)
  if(needPass) {
    //todo check if result is hash: 1. string, 2. length, 3. hex format
  }
  else{
    expect((isErrorInResult) ? response.result : response.message).to.contains(expectedError)
  }
}
...
function checkGetTxResponse(response, needPass, isErrorInResult, expectedError){

}

function compareTxWithParams(commonParams, tx){
      ...
      describe('is working', function () {
        ...
        describe('demo', function () {
          ...
          after(async function() {
            //set timeout to ensure the next test which use the same sender address can pass the test
            return await utility.timeout(10000)
          })
