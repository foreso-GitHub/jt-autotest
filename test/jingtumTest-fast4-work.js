    ...
    describe('测试模式: ' + server.getName(), function () {
      ...
      describe('is working', function () {
        ...
        describe('demo', async function () {
          ...
          //   await executeSendTx(server)
          //   logger.debug("before: " + JSON.stringify(desc.testCases[0].response))
          ...
          for(let i = 0; i < 5; i++){
            it('0010\t发起有效交易:' + i, function () {
              logger.debug("case: " + i)
              logger.debug("desc: " + JSON.stringify(desc.testCases[0].response))
...
//region execute send tx
 function prepare(server){
  executeSendTx(server).then(async function(result){
    return result
  })
}
...
async function executeSendTx(server){
  return new Promise(async function (resolve){
    let initDesc = createCases()
    await executeDescribe(server, initDesc).then(function(description){
...
async function testFastSend(server){
  ...
  desc = description
  logger.debug("desc in testFastSend: " + JSON.stringify(desc))
  await executeDescribe(server, description).then(function(description){
    // desc = description
    logger.debug("desc in testFastSend2: " + JSON.stringify(desc.testCases[0]))
    return description
  })
  // return description
}
