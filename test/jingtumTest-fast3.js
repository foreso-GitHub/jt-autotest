      ...
      describe('is working', function () {
        ...
        describe('demo', async function () {
          ...
          before(async function() {
            //set timeout to ensure the next test which use the same sender address can pass the test
            // return await utility.timeout(data.defaultBlockTime)
            logger.debug("????1" + JSON.stringify(description))
            description = await prepare(server)
            logger.debug("????2" + JSON.stringify(description))
          ...
          // let des = await testFastSend(server)
          //
          // await utility.timeout(5000)

          logger.debug("????3" + JSON.stringify(description))
...
async function testFastSend(server){
  ...
  let aaa = await executeDescribe(server, description)
  return description
}

function sendTxWithSequence2(server, txParams){

  // Promise.resolve(getSequence(server, txParams.from)).then(function (sequence){
  //   server.responseSendTxWithSequence(
  //       txParams.from,
  //       txParams.secret,
  //       sequence,
  //       txParams.to,
